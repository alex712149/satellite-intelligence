...
The first real run downloads OpenCLIP's pretrained `ViT-B-32` / `openai`
weights once (a few hundred MB) and caches them under `~/.cache/clip`
(or `open_clip`'s default cache dir). After that first download, the
model loads from the local cache with **no network access required** —
this is what satisfies the PS's offline-demonstration constraint.
Stage this download *before* the demo, on a machine with internet, then
copy the cache directory to the offline demo machine.
...
## Swapping in RemoteCLIP instead of generic OpenCLIP
...
1. Stage the RemoteCLIP checkpoint file locally.
2. In `app/config.py`, point `CLIP_MODEL_NAME` / `CLIP_PRETRAINED` at
   it (open_clip supports loading from a local `.pt` path directly as
   the `pretrained` argument).
3. Update `EMBEDDING_DIM` if the checkpoint's output dimension differs
   from 512.
4. Delete `data/index/tiles.faiss` and `data/catalog.sqlite` and
   re-ingest — embeddings from two different backbones are not
   comparable in the same index.
