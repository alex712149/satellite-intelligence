...
def text_search(query: str, top_k: int = 10, date_from: str = None, date_to: str = None,
                 sensor: str = None) -> list[SearchResult]:
    """Stage 9. Example: text_search('newly built structures near a river')"""
    query_vec = embed_text(query)
    index = VectorIndex()
    # Over-fetch before filtering, since AOI/date/sensor filters are
    # applied post-hoc against SQLite rather than inside FAISS.
    ids, scores = index.search(query_vec, top_k=max(top_k * 5, 50))
...
def image_search_by_upload(pil_image: Image.Image, top_k: int = 10) -> list[SearchResult]:
    ...
    index = VectorIndex()
    ids, scores = index.search(query_vec, top_k=top_k)
    return _rows_to_results(ids, scores)
